...
import { Form, Select, InputNumber, Button, Card, Typography, Alert } from 'antd';
...
const LoanCalculator = () => {
  ...
  const calculateLoan = (values) => {
    const { category, subcategory, amount, initialDeposit, period } = values;
    const principalAmount = amount - initialDeposit;
    
    // Simple interest calculation (mock calculation)
                ...
                validator(_, value) {
                  ...
                  if (maxLoan && value > maxLoan) {
                    return Promise.reject(
                      new Error(`Maximum loan amount is PKR ${maxLoan.toLocaleString()}`)
                    );
                  }
        ...
        {calculationResult && (
          <Card className="mt-4 bg-blue-50">
            ...
            <div className="space-y-2">
              <Text>Principal Amount: PKR {calculationResult.principalAmount.toLocaleString()}</Text>
              <Text>Total Interest: PKR {calculationResult.totalInterest.toLocaleString()}</Text>
              <Text>Monthly Payment: PKR {calculationResult.monthlyPayment.toLocaleString()}</Text>
              <Text strong>Total Payback: PKR {calculationResult.totalPayback.toLocaleString()}</Text>
            </div>
          </Card>
        )}
      </Card>
